#!/usr/bin/env python3
"""Run Phase 5 CUDA confirmation grid on Colab A100 (no notebook required).

Phase 5 replicates the Phase 3 frontier grid on CUDA and compares directional
trend stability against the MPS/Metal local baseline via ``compare-backends``.

Usage on Colab (in a code cell):

    !git clone https://github.com/Shiv-aurora/creative-ai-research.git
    %cd creative-ai-research
    !python scripts/run_phase5_colab_a100.py \
        --install-deps --download-models \
        --output-root outputs/phase5_cuda_confirm

If the repo is private, clone with a PAT first (e.g. via Colab secret
GITHUB_TOKEN).
"""
...
# ── HuggingFace GGUF sources for all Phase 5 models ──────────────────────────
DEFAULT_HF_GGUFS = {
    "gemma-2b": (
    ...
    "mistral-7b-v0.3": (
        # Base Mistral-7B-v0.3 GGUF is less commonly hosted than the Instruct
        # variant.  The QuantFactory repo below is a verified community quant;
        # if it becomes unavailable, override with:
        #   --hf-model 'mistral-7b-v0.3=REPO:FILE'
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Run Phase 5 CUDA confirmation grid on Colab/A100 without a notebook. "
            "Replicates the Phase 3 frontier grid on CUDA and runs compare-backends "
            "against the local MPS frontier. "
    ...
    p.add_argument("--output-root", default="outputs/phase5_cuda_confirm")
    p.add_argument("--backend", default="llama_cpp")

    # ── Grid axes (match phase5_cuda_confirm.json) ────────────────────────────
    p.add_argument(
        "--models",
        default="gemma-2b,gemma-2b-it,qwen2.5-3b,qwen2.5-3b-instruct,mistral-7b-v0.3,mistral-7b-instruct-v0.3",
    )
    p.add_argument(
        "--methods",
        default="one_shot,best_of_k_one_shot,restlessness_best,restlessness_last_iter",
    )
    p.add_argument("--tasks", default="cdat,aut,dat")
    p.add_argument("--temperatures", default="0.7")
    p.add_argument("--seeds", default="11,37")
    p.add_argument("--limit-cues", type=int, default=20)
    p.add_argument("--limit-aut", type=int, default=15)
    p.add_argument("--cue-offset", type=int, default=0)
    p.add_argument("--aut-offset", type=int, default=0)
    p.add_argument("--dat-repeats", type=int, default=1)
    p.add_argument("--max-tokens", type=int, default=256)
    p.add_argument("--restlessness-k", type=int, default=3)
    p.add_argument("--best-of-k", type=int, default=4)
    p.add_argument("--token-budget", type=int, default=800, help="Token budget for best-of-N analysis.")
    ...
    p.add_argument("--compute-matched-k", type=int, default=4)
    ...
    p.add_argument(
        ...
        default="",
        help=(
            "Path to local (MPS) frontier_analysis.json for compare-backends. "
            "If empty, compare-backends is skipped."
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    log(f"Phase 5 CUDA confirm — {len(model_ids)} models, output={output_root}")
    ...
    session_id = f"phase5-cuda-confirm-{int(time.time())}"
    ...
    # ── Step 4: analysis (frontier + homogeneity + compare-backends) ──────────
    if not args.skip_analysis:
        ...
        # Homogeneity audit (pooled)
        run(
            [*cli, "audit-homogeneity", "--runs", str(runs_dir / "runs.jsonl"), "--output-dir", str(analysis_dir)],
            env=env,
            cwd=root,
        )

        # Homogeneity audit (task-stratified)
        run(
            [
                *cli,
                "audit-homogeneity",
        ...
        # Compare-backends (if local frontier provided)
        local_frontier = args.local_frontier.strip()
        if local_frontier:
            cuda_frontier = str(analysis_dir / "frontier_analysis.json")
            parity_output = str(analysis_dir / "backend_parity.json")
            run(
                [
                    *cli,
                    "compare-backends",
            ...
            log(f"Backend parity report: {parity_output}")
        else:
            log(
                "Skipping compare-backends (no --local-frontier given). "
                "Run manually later with:\n"
                "  python -m creativeai.cli compare-backends \\\n"
                "    --local-frontier outputs/phase3_main_*/analysis/frontier_analysis.json \\\n"
                "    --cuda-frontier outputs/phase5_cuda_confirm/analysis/frontier_analysis.json \\\n"
                "    --output outputs/phase5_cuda_confirm/analysis/backend_parity.json"
            )

    # ── Summary ───────────────────────────────────────────────────────────────
    summary = {
        "phase": "phase5_cuda_confirm",
        "session_id": session_id,
        "output_root": str(output_root),
        "runs": str(runs_dir),
        "scores": str(scores_dir),
        "analysis": str(analysis_dir),
        "frontier_json": str(analysis_dir / "frontier_analysis.json"),
        "frontier_plot": str(analysis_dir / "frontier.png"),
        "homogeneity": str(analysis_dir / "homogeneity_audit.json"),
        "experiment_manifest": str(runs_dir / "experiment_manifest.json"),
        "model_path_map": str(model_path_map),
        "models": model_ids,
        "grid_estimate": f"{len(model_ids)} models × {len(args.methods.split(','))} methods × {len(args.seeds.split(','))} seeds",
    }
    log("Phase 5 CUDA confirmation complete.")
